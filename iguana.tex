...
\begin{document}
% Helicóptero
\def\title{Universidad Nacional de Córdoba - Helicóptero}
.\\[0.2cm]
\centering{\LARGE\textbf{Helicóptero}} \\[0.5cm]
\centering{\includegraphics[width=5.5cm]{img/helicopterito.png}}
\tableofcontents\newpage
...
\subsection{Identities}
{
	...
	$log(1-x^k) = - \sum_{j=1}^{\infty} \frac{x^{kj}}{j}$
}
...
\subsection{Minimum Enclosing Cricle (+ Circumcenter)}
...
\subsection{Brute Force stress test checker}
\cppfile{other/BF_checker.py}
\subsection{Interactive problem grader template}
\cppfile{other/interactive_grader.cpp}
